...
describe('JSONReport', () => {
  test('it represents jsonReport.json',async () => {
    await copy();
    const report = new JSONReport(resolve('backstop', 'failed', 'backstop_data', 'json_report', 'jsonReport.json'));
  ...
  test('it merges test result', async () => {
    await copy();
    const r1 = new JSONReport(resolve('backstop', 'tablet_success_sp_failed', 'backstop_data', 'json_report', 'jsonReport.json'));
    const r2 = new JSONReport(resolve('backstop', 'pass', 'backstop_data', 'json_report', 'jsonReport.json'));
